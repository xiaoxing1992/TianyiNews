...
public class HomeFragment extends BaseFragment {
    ...
    protected void initData() {
      ...
      /*  for (int i = 0; i < tabtitles.length; i++) {
       ...
       /* home_tab_layout.setTabMode(TabLayout.MODE_SCROLLABLE);
        for (int i = 0; i < tabTitleList.size(); i++) {
            home_tab_layout.addTab(home_tab_layout.newTab().setText(tabTitleList.get(i)));
        }


        home_tab_layout.setupWithViewPager(home_view_pager);
        home_tab_layout.setTabsFromPagerAdapter(adapter);*/
